!pip install bitsandbytes>=0.45.5 --quiet
!pip install accelerate>=1.7.0 --quiet
!pip install transformers>=4.51.3 --quiet
!pip install flash-attn --upgrade --quiet
import os
import time
import re
import logging
...
def get_available_models() -> Dict[str, ModelConfig]:
    return {
        "gemma-3n-4b-preview": ModelConfig(
            name="Gemma 3n 4B Preview",
            provider="huggingface",
...
def call_huggingface(prompt: str, config: ModelConfig) -> str:
    if config._model is None or config._tokenizer is None:
        load_huggingface_model(config)
    inputs = config._tokenizer(prompt, return_tensors="pt", truncation=True).to(config._model.device)
    ...
    text = config._tokenizer.decode(outputs[0], skip_special_tokens=True)
