...
def train_model_sparse(model, X_train, y_train, X_val, y_val):
    """
    Train model using sparse matrices
    """
...
def tune_hyperparameters_sparse(model, X_train, y_train, X_val, y_val, hyperparam_grid):
    """
    Tune hyperparameters using sparse matrices
    """
    best_score = 0
    best_params = None
...
def test_model_sparse(model, best_params, X_train, y_train, X_test, y_test):
    """
    Test model using sparse matrices
    """
    ...
    models = (
        {
            ...
            "scale": False,
            "requires_non_negative": True  # Added flag for models requiring non-negative values
        },
        # "SVM",
        {
                ...
                for model_config in models:
                    ...
                    if pca_dim == 0:
                        ...
                        try:
                            ...
                            # This is more appropriate for text data and preserves non-negativity for NB
                    ...
                    best_params, val_score = tune_hyperparameters_sparse(
                        model, X_train_scaled, y_train, X_val_scaled, y_val, 
                        model_config['hyperparameters']
                    )
                    print(f"Best parameters: {best_params}, validation score: {val_score:.4f}")
